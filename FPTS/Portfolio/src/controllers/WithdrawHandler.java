 ...
 *
 */
public class WithdrawHandler implements EventHandler<ActionEvent> {
	private MediatorController mc;
	private Account account;
	private double amountWithdrawn;
	
	public WithdrawHandler(MediatorController mc, Account a) {
		this.mc = mc;
		this.account = a;
	}
	...
	public void handle(ActionEvent event) {
		if(showWithdrawNumber()){
			withdraw();
		}
	}
	
	private boolean showWithdrawNumber(){
		...
		t.setContentText("How much would you like to Withdraw?");
		...
		if(correct){
			...
			alert.setContentText("Withdraw " + df.format(amount) + "?");
	...
	private void withdraw(){
		if(mc.getPortfolio().removeMoney(account, amountWithdrawn)){
			// Update the Accounts List
			mc.performManageAccounts();
		}
		else{
			Alert cantWithdrawAlert = new Alert(AlertType.INFORMATION);
			cantWithdrawAlert.setTitle("Information Dialog");
			cantWithdrawAlert.setHeaderText(null);
			cantWithdrawAlert.setContentText("You don't have that much money to withdraw.");
			cantWithdrawAlert.showAndWait();
		}
	}
}
